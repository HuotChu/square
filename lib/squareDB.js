...
define(['./eventHub'], function(eventHub) {
    ...
    var min = function (dataArray) {
        // todo: create sortObject from passed in dataArray <result of select>
        // sortObject contains the columns to perform the aggregate function on
        var columnName, sortArray, sortObject = createSortObject(dataArray);

        for (columnName in sortObject) {
            if (sortObject.hasOwnProperty(columnName)) {
                sortArray = sortObject[columnName];
                sortObject[columnName] = sortArray.reduce(function (previous, current) {
