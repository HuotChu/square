...
/*        var subscribers = html.querySelectorAll('[data-subscribe]'),
        ...
        if (model && model._domDataNodes) {
            html.querySelectorAll('[data-id]').forEach(function (el) {
                ...
                for (; path.length >= 0; node = path.shift()) {
                    if (!path.length) {
                        nodeObj = {
                            node: el,
                            childNodes: {}
                    ...
                    if (path.length === cachedLen - 1) {
                        // root node
                        currentNode = model._domDataNodes[node];
                        if (!currentNode) {
                            currentNode = model._domDataNodes[node] = nodeObj;
                        }
                    } else {
                        // child node
                        currentNode = currentNode.childNodes[node];
                        if (!currentNode) {
                            currentNode = currentNode.childNodes[node] = nodeObj;
                        }
                    }
                }
            });
        ...
        if (model && model._dataSpy) {
            html.querySelectorAll('[data-bind]').forEach(function (el) {
                var dataTopic = el.getAttribute('data-bind').replace(/\./g, '_'),
                    modelEvent = el.getAttribute('data-model-event'),
                    modelCallBack = el.getAttribute('data-model-callback'),
                    modelUpdate = el.getAttribute('data-model-update'),
                    domId = el.getAttribute('data-id'),
                    modelDom = model._domDataNodes.getNode(domId),
                    e = model._dataSpy[modelEvent],
                ...
                if (e) {
                    ...
                    if (modelCallBack) {
                        spy.callBack = modelCallBack;
                    }
                    if (modelUpdate) {
                        spy.update = modelUpdate;
                    }

                    topic = e[dataTopic];

                    if (!topic) {
                        e[dataTopic] = model.newDataArray().push(spy);
                    } else {
                        e[dataTopic].push(spy);
                    }
                }
            });
        }
