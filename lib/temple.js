    ...
    var _build = function (html, data, toDom) {
        ...
        if (data) {
            html = html.replace(_re1, function (match, p1) {
                ...
                } else if (returnVal instanceof Function === false) {
                    returnVal = returnVal[node];
                } else {
            ...
            var createRepeatingElements = function (path, elm) {
                var dataObjects = data.getNodeFromPath(path) || data[path] || [],
                    dataObj,
                    str = '',
                    i = 0,
        ...
        var domDataNodes = data._domDataNodes || data._model._domDataNodes;

        if (domDataNodes) {
            var setDomDataNodes = function (el) {
                var id = el.getAttribute('data-id'),
                    ...
                    node = path.shift(),
                    currentNode = domDataNodes[node],
                    len;

                    if (!currentNode) {
                        domDataNodes[node] = nodeObj;
                        currentNode = domDataNodes[node];
                    }

                    for (; node = path.shift() && len > 0;) {
                        len = node.length;
                        currentNode = currentNode.childNodes[node];
                        if (!currentNode) {
                            currentNode.childNodes[node] = nodeObj;
                        }
                    }
                    ...
                    currentNode[node] = nodeObj;
            },
                dataNodes = html.querySelectorAll('[data-id]');

            for (i = 0, len = dataNodes.length; i < len; ++i) {
                setDomDataNodes(dataNodes[i]);
            }
        }
        ...
        if (data._dataSpy) {
            var setDataSpies = function (el) {
                var dataTopic = el.getAttribute('data-spy').replace(/\./g, '_'),
                    ...
                    modelUpdate = el.getAttribute('data-update'),
                    domId = el.getAttribute('data-id'),
                    modelDom = data._domDataNodes.getNode(domId),
                    e = data._dataSpy[modelEvent],
                    spyName = dataTopic + Date.now(),
                    spy = data.newDataObject().set({
                        node: el,
                        id  : spyName
                    }),
                    topic;
                ...
                if (e) {
                    ...
                    if (modelUpdate) {
                        spy.set(modelUpdate, 'update');
                    }

                    topic = e[dataTopic];

                    if (!topic) {
                        e[dataTopic] = data.newDataArray().add(spy);
                    } else {
                        e[dataTopic].add(spy);
                    }
                }
