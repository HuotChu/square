...
define(function () {
    ...
    var _build = function (html, data, toDom) {
        ...
        if (data) {
            html = html.replace(_re1, function () {
                var matched = RegExp.$1,
                    ...
                    returnVal = model ? model.getNodeFromPath(matched) : data[root];
        ...
        for (i = 0, len = dataBound.length; i < len; ++i) {
            ...
            eventHandler = f(path, topicName, data.modelName);
