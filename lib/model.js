define(function () {
    ...
    Model.__proto__ = {
            ...
            getNode: function (sPath) {
                ...
                for (; path.length >= 0; node = path.shift()) {
                    ...
                    if (path.length === cachedLen - 1) {
                        currentNode = this._domDataNodes[node];
                    } else {
                        // not first node
                        currentNode = node.childNodes[node];
                    }
                }
        ...
        _update: function (updateEvent, dataName, changedVal) {
            var e = this._dataSpy[updateEvent][dataName] || [],
                changed = updateEvent !== 'change' ? this._dataSpy.change[dataName] : [],
                notify = function (watcher) {
                    ...
                    if (callBack) {
                        callBack.call(domNode, changedVal, updateEvent, dataName, this);
                    }
                    if (update) {
                        switch (update) {
                            case 'checked':
                ...
                dataObject = Object.create(Object.prototype, {
                    ...
                    'set': {
                        value: function (val, prop) {
                            ...
                            if (path && nodeName) {
                                model._update(e, path + '.' + nodeName + (prop ? '.' + prop : ''), val);
                            }
                ...
                dataArray = Object.create(Array.prototype, {
                    ...
                    '_update': {
                        value: function (val, evt) {
                            ...
                            if (path && arrayName) {
                                model._update(e, path + '.' + arrayName, val);
                            }
                        },
                        enumerable: false,
                        writable: false,
                    ...
                    'cloneArray': {
                        value: function (inputArr) {
                            ...
                            if (path && arrayName) {
                                model._update(e, path + '.' + arrayName, this);
                            }
