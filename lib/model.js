define(['notify'], function (notify) {
    ...
    Model.prototype = {
        ...
        create: function (modelName, propsObj) {
            var model = _models[modelName] = Object.create(new Model()),
                prop;
        ...
        add: function (topicName, dataObj, customEventName, modelName) {
            var model = this.getModel(modelName || this.modelName),
                topic = model[topicName] || [];

            topic.push(dataObj);

            notify.publish(topicName + (customEventName || ''));
        ...
        update: function (path, newVal, customEventName, modelName) {
            ...
            for (i; i < len; ++i) {
                ...
                // this allows update to add without overwriting like insert does
                topic = topic[targetNode] || (function() {
                        topic[targetNode] = {};
                        return topic[targetNode];
                    }());
            }
        ...
        /**
         * selectAll returns all objects from a data topic array which match the compFunc
         * @param topicName {String} Name of a data topic array
         * @param compFunc {Function} Test object property values against a comparator !Function must return either the passing object or boolean false
         * @param modelName [String] Name of model to use (or current model)
         * @returns {boolean|Object} Returns the found object or boolean false
         */
        selectAll: function (topicName, compFunc, modelName) {
            var model = this.getModel(modelName || this.modelName),
                topic = model[topicName] || [],
                found = [];

            topic.forEach(function (obj) {
                if (compFunc(obj) !== false) {
                    found.push(obj);
                }
            });
