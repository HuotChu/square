...
define(['./aggregates', './comparator'], function (aggregates, comparator) {
    ...
    var go = function (queryObject) {
        var o = queryObject,
            ...
            response, columnName, column, initial,
            i, j, jLen, and, andi, or, ori, ndx;
        ...
        if (db.prototype && db.prototype.dispatch) {
            // process model events
            var eventName = table._event;

            selects.forEach(function (colName) {
                var details = {
                    value: Array.from(response)
                };

                db.prototype.dispatch(eventName + '.' + colName + '.read', details);
            });
        }
