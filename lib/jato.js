    ...
    DataCollection.prototype = {
        ...
        'add': function (dataObj) {
            var event = this.event,
                model = this._model,
                childEvent = function () { return new Date().getTime() },
                addObject = function (o) {
                    var detail;

                    if (!(o instanceof DataObject)) {
                        o = new DataObject().set(o, false, true);
                    }
                    o._model = model;
                    o.event = event + '.' + childEvent();
                    detail = {
                        'target': this,
                        'type': 'add',
                        'value': o
            ...
            if (dataObj instanceof Array) {
                ...
                for (i; i < len; ++i) {
                    dataObj = dataArray[i];
                    returnVal.push(addObject(dataObj));
                }
            } else {
                returnVal = addObject(dataObj);
            }

            return returnVal;
            // this is the event name to listen to for changes in this DataObject
            // or an array of event names if objects were added as an array
        },
        'drop': function (ndx) {
