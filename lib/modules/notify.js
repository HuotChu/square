...
define(function () {
    ...
    var setRoute = function (topic, paramObj) {
        _whiteList[topic] = paramObj || true;
    };

    var _autoPublish = function (hashChangeEvent) {
        var hash = window.location.hash,
            topic = hash ? hash.match(/[^\/]/gi).join('') : hash,
            newURL = hashChangeEvent.newURL,
            oldURL = hashChangeEvent.oldURL;

        if (hash && _whiteList[topic]) {
            publish(topic);
        }
        /*else if (!hash) {
            // no hash, should we default???
        }*/
    };

    window.addEventListener("hashchange", _autoPublish, false);
    ...
    var subscribe = function(topic, callBackObj) {
        ...
        topics[topic].push({
            "callBack": callBackObj instanceof Function ? callBackObj : callBackObj.callBack,
            "context": callBackObj.context || window,
            "id": id
        });
    ...
    var update = function (eventName, target) {
        this.publish(eventName + '?' + objToArgStr(target));
    };
